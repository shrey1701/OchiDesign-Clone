...
function Navbar() {
  const [isScrolledUp, setIsScrolledUp] = useState(true); // Initially show the navbar
  ...
  return (
    <div
      className={`navb w-full px-[6.5vw] py-[1.25vw] flex justify-between transition-all duration-300 ${
        isScrolledUp
          ? "fixed top-0 z-[999] backdrop-blur-lg bg-opacity-60 bg-zinc-900"
          : "absolute top-0"
      }`}
    >
      <div className="logo">
      ...
      <div className="links flex gap-[2.5vw]">
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
          (item, index) => (
            <a
              key={index}
              className={`text-[1.25vw] capitalize font-light ${
                index === 4 && "ml-[8vw]"
              }`}
            >
              {item}
